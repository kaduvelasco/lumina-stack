...
remover_bancos_de_dados() {
    ...
    local DBS
    DBS=$(executar_mysql -e "SHOW DATABASES;" 2>/dev/null \
...
otimizar_mariadb_moodle() {
    ...
    # Tenta detectar RAM automaticamente
    local TOTAL_RAM_MB
    ...
    case "$escolha_ram" in
        ...
        *)
            echo -e "   ${AMARELO}⚠️  Opção inválida. Usando 1/3 como padrão.${RESET}"
            BUFFER_POOL_MB=$(( TOTAL_RAM_MB / 3 ))
            ;;
    esac
